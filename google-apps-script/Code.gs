...
function doGet(){return json_({ok:true,service:'UTOP-3Dv3 Cloud',version:'1.5.3'});}
function doPost(e){
  try{
    const body=JSON.parse((e&&e.postData&&e.postData.contents)||'{}');
    const action=String(body.action||'ping');
    if(action==='ping')return json_({ok:true,service:'UTOP-3Dv3 Cloud',version:'1.5.3',time:new Date().toISOString()});
...
function selfTest_(){
  ...
  return {ok:true,service:'UTOP-3Dv3 Cloud',version:'1.5.3',folderId:folder.getId(),folderName:folder.getName(),spreadsheetId:ss.getId(),spreadsheetName:ss.getName(),sheetName:sh.getName(),projects:listProjects_().length,time:new Date().toISOString()};
}
