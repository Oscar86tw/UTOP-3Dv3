...
export function runFunctionStateAudit(){
  ...
  add(checks,'設備清單',devices.length>0,`${devices.length} 台設備`);
  ...
  try{const k='__utop_validate__';localStorage.setItem(k,'1');localStorage.removeItem(k);add(checks,'本機儲存',true,'localStorage 可讀寫');}catch(err){add(checks,'本機儲存',false,err?.message||String(err));}
