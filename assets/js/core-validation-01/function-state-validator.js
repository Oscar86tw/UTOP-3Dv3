...
import {verifyConnectionActionChains} from '../core-logic-01/connection-runtime.js?v=1.7.14';
import {APP_VERSION,APP_VERSION_LABEL} from '../core-version-01/version-info.js?v=1.7.14';
...
export function runFunctionStateAudit(){
  ...
  const metaEl=document.getElementById('projectMeta');const meta=metaEl?.textContent||'';const metaVersion=metaEl?.dataset?.version||'';const domVersion=document.documentElement.dataset.utopVersion||'';add(checks,'版本同步',domVersion===APP_VERSION&&metaVersion===APP_VERSION,`首頁 ${meta||'-'} [${metaVersion||'-'}] / Runtime ${domVersion||'-'} / Expected ${APP_VERSION_LABEL}`);
  const build=window.__utopBuild||{};const indexOk=!build.indexVersion||build.indexVersion===APP_VERSION;const manifestLoaded=build.manifestState==='loaded';const manifestOk=!manifestLoaded||build.manifestVersion===APP_VERSION;const deployOk=indexOk&&manifestOk&&build.consistent!==false;const manifestDetail=manifestLoaded?(build.manifestVersion||'-'):(build.manifestState==='error'?`無法讀取 (${build.manifestError||'unknown'})`:'讀取中');add(checks,'部署檔案一致性',deployOk,`Index ${build.indexVersion||'-'} / Runtime ${APP_VERSION} / Manifest ${manifestDetail}`);
