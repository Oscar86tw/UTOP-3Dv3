...
import {verifyConnectionActionChains} from '../core-logic-01/connection-runtime.js?v=1.7.10';
import {APP_VERSION,APP_VERSION_LABEL} from '../core-version-01/version-info.js?v=1.7.10';
...
export function runFunctionStateAudit(){
  ...
  const build=window.__utopBuild||{};add(checks,'部署檔案一致性',build.consistent!==false&&(!build.indexVersion||build.indexVersion===APP_VERSION)&&(!build.manifestVersion||build.manifestVersion===APP_VERSION),`Index ${build.indexVersion||'-'} / Runtime ${APP_VERSION} / Manifest ${build.manifestVersion||'-'}`);
