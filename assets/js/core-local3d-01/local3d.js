...
export function createLocal3D(host,callbacks={}){
  ...
  function drawDevice(d,trace){const t=getDeviceTransform(d.id);if(!t)return;const p=project(t.x,t.y,t.z);const onTrace=!state.signalTrace?.enabled||trace.devices.includes(d.id);ctx.globalAlpha=onTrace?1:.18;ctx.fillStyle=colorFor(d.type);ctx.strokeStyle=d.id===selected?'#111827':'#ffffff';ctx.lineWidth=(d.id===selected?4:2)*(devicePixelRatio||1);const size=(d.type.toLowerCase().includes('loop')?20:12)*(devicePixelRatio||1);ctx.beginPath();ctx.roundRect(p.x-size,p.y-size*1.2,size*2,size*2.4,5*(devicePixelRatio||1));ctx.fill();ctx.stroke();ctx.globalAlpha=1;ctx.fillStyle='#111827';ctx.font=`${12*(devicePixelRatio||1)}px sans-serif`;ctx.textAlign='center';ctx.fillText(labelFor(d),p.x,p.y-size*1.6);const rt=getRuntime(d.id);ctx.font=`${10*(devicePixelRatio||1)}px sans-serif`;ctx.fillText(rt?.status||d.state||'',p.x,p.y+size*1.8);}
