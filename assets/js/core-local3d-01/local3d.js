...
export function createLocal3D(host,callbacks={}){
  ...
  let raf=0,selected=state.selectedDevice||devices[0]?.id||null,mode=state.editor.mode||'select',zoom=1,panX=0,panY=0,drag=null,car={x:0,z:15,a:0,s:0,type:state.simulator.vehicleType||'car'},keys=new Set(),follow=false;
  ...
  function drawCar(){const p=project(car.x,.2,car.z),dpr=(devicePixelRatio||1),bike=(car.type||state.simulator.vehicleType)==='motorcycle';ctx.save();ctx.translate(p.x,p.y);ctx.rotate(-car.a);ctx.fillStyle='#0f766e';if(bike){ctx.fillRect(-5*dpr,-13*dpr,10*dpr,26*dpr);ctx.fillStyle='#111827';ctx.beginPath();ctx.arc(0,-12*dpr,4*dpr,0,Math.PI*2);ctx.arc(0,12*dpr,4*dpr,0,Math.PI*2);ctx.fill();}else{ctx.fillRect(-10*dpr,-18*dpr,20*dpr,36*dpr);ctx.fillStyle='#bae6fd';ctx.fillRect(-7*dpr,-10*dpr,14*dpr,11*dpr);}ctx.restore();}
  function frame(){
