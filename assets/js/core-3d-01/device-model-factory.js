...
function makeBarrier(THREE,mats,settings){
  ...
  const serviceDoor=box(THREE,root,.245,.56,.012,mats.dark,0,.47,.148);
  const lock=cyl(THREE,root,.014,.014,.014,mats.gray,.083,.51,.158,18,Math.PI/2);
  for(let i=0;i<5;i++)box(THREE,root,.055,.008,.006,mats.gray,-.085+i*.043,.30,.158);
  for(let i=0;i<4;i++)ledBulb(THREE,root,i===0?mats.green:mats.gray,-.075+i*.05,.86,.154,.011);
  box(THREE,root,.22,.08,.24,mats.dark,0,1.03,0);
  const pivotCap=cyl(THREE,root,.055,.055,.055,mats.dark,-.14,.88,0,24,0,0,Math.PI/2);
  ...
  const endCap=box(THREE,pivot,.07,.09,.09,mats.dark,-boomLen+.025,0,0);
  root.add(pivot);
  root.userData.body=body; root.userData.serviceDoor=serviceDoor; root.userData.lock=lock; root.userData.pivotCap=pivotCap; root.userData.barrierPivot=pivot; root.userData.barrierArm=arm; root.userData.endCap=endCap;
  return clearApply(root,(s,def)=>{const w=num(s,'width',def.width||.34),h=num(s,'height',def.height||1.03),d=num(s,'depth',def.depth||.28),boom=num(s,'boomLength',def.boomLength||2.5);setScale(body,w/(def.width||.34),h/(def.height||1.03),d/(def.depth||.28));arm.scale.x=boom/(def.boomLength||2.5);endCap.position.x=-boom+.025; const side=String(s.armSide||'left').toLowerCase(); pivot.scale.x=side==='right'?-1:1;});
}
function makeUhf(THREE,mats,settings){
  const root=new THREE.Group();
  const mountH=num(settings,'mountHeight',2.4); pole(THREE,root,mats,mountH,0,0,.045);
  const bracket=box(THREE,root,.075,.18,.085,mats.dark,0,mountH-.12,.01);
  const body=box(THREE,root,.228,.228,.052,mats.white,0,mountH,.045);
  const face=box(THREE,root,.198,.198,.008,mats.blue,0,mountH,.072);
  box(THREE,root,.112,.012,.004,mats.dark,0,mountH+.006,.079);
  box(THREE,root,.024,.024,.004,mats.green,.075,mountH-.075,.079);
  const hood=box(THREE,root,.235,.018,.06,mats.gray,0,mountH+.123,.044);
  for(const x of [-.09,.09])for(const y of [-.09,.09])cyl(THREE,root,.006,.006,.008,mats.dark,x,mountH+y,.081,10,Math.PI/2);
...
function makeCardReader(THREE,mats){const root=new THREE.Group(); const post=box(THREE,root,.12,1.08,.12,mats.dark,0,.54,0); const reader=box(THREE,root,.115,.245,.042,mats.gray,0,.82,.09); box(THREE,root,.075,.115,.007,mats.dark,0,.84,.113); ledBulb(THREE,root,mats.green,0,.94,.121,.011); box(THREE,root,.055,.012,.004,mats.blue,0,.76,.119); for(let i=0;i<3;i++)box(THREE,root,.045,.006,.004,mats.dark,0,.71-i*.015,.119); root.userData.post=post; root.userData.reader=reader; return clearApply(root,()=>{});}
...
function makeIpCamera(THREE,mats){const root=new THREE.Group(); cyl(THREE,root,.19,.19,.065,mats.white,0,.032,0); const trim=cyl(THREE,root,.175,.175,.022,mats.gray,0,.072,0); const dome=sph(THREE,root,.17,mats.dark,0,.073,0,0,Math.PI*2,0,Math.PI/2); dome.rotation.x=Math.PI; lens(THREE,root,mats,0,.070,.139,.045); for(let i=0;i<6;i++)ledBulb(THREE,root,mats.red,-.06+i*.024,.06,.143,.005); root.userData.cameraBody=dome; root.userData.trim=trim; return clearApply(root,()=>{});}
...
function makeTraffic(THREE,mats,settings){const root=new THREE.Group(); const h=num(settings,'height',1.5); pole(THREE,root,mats,h-.4); const boxy=box(THREE,root,.29,.57,.16,mats.dark,0,h-.22,0); const hood1=box(THREE,root,.18,.045,.11,mats.dark,0,h+.015,.115,0,0,-.08); const hood2=box(THREE,root,.18,.045,.11,mats.dark,0,h-.275,.115,0,0,-.08); const red=ledBulb(THREE,root,mats.red,0,h-.06,.09,.06),green=ledBulb(THREE,root,mats.green,0,h-.35,.09,.06); root.userData.trafficRed=red; root.userData.trafficGreen=green; root.userData.body=boxy; root.userData.hoods=[hood1,hood2]; return clearApply(root,()=>{});}
